		...
		)
        {
			...
			try
			{
				...
				if (__instance.isEnabled && !__instance.isPaused)
				{
					...
					if (___isGrounded)
					{
						num = Mathf.Lerp(___rigidbody2D.velocity.x, ___axisHorizontal * ___speed * 1.2f, Time.deltaTime * 15f);
					...
					{
						num = Mathf.Lerp(___rigidbody2D.velocity.x, ___axisHorizontal * ___speed * 1.2f, Time.deltaTime * 10f);
					}
					...
					if (___isGrounded)
					{
						...
						if (raycastHit2D2.collider && !raycastHit2D2.collider.isTrigger)
						{
							...
							if (__instance.hitPushable && __instance.hitPushable.enabled && !__instance.hitPushable.IsFloating())
							{
								...
								float num6 = Mathf.Lerp(component12.position.x, num5, ___pushTimer);
					...
					if ((___isGrounded || ___isNotGroundedTimer < ___timeAfterFallPlayerCanJump) && ___axisJump > 0f && ___jumpReset && !___forceJump && !ColourWheelTrigger.instance.maskActive)
					{
						___isGrounded = false;
						___jumpReset = false;
						Collider2D collider = ___highestHit.collider;
						JumpThroughPlatformNew exists = null;
						if (collider)
						{
							exists = collider.GetComponent<JumpThroughPlatformNew>();
						}
						if (exists && ___axisVertical < ___jumpThroughAxisY)
						{
							__instance.forceJump = true;
						}
						else
					...
					if (!(___isGrounded || ___isNotGroundedTimer < ___timeAfterFallPlayerCanJump) && ___axisJump > 0f && Main.dbljReset && Main.dbljRelease && !Main.forceDblj && !ColourWheelTrigger.instance.maskActive)
					{
						Main.dbljReset = false;
						Collider2D collider = ___highestHit.collider;
						JumpThroughPlatformNew exists = null;
						if (collider)
						{
				...
				num2 -= Mathf.Lerp(___gravityForce, 0f, Mathf.Clamp(num2, -10f, 0f) / -10f);
